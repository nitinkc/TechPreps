...
> Purpose: Organize all raw interview questions you provided into clear categories with explicit questions (Q) followed by concise layered answers (A). The existing `L2_Technical_Interview_Guide.md` remains unchanged for deep-dive code samples. This file is your high-level revision map; use it first, then jump to original guide for detailed implementations.
...
A: Use Resilience4j: `@CircuitBreaker(name="service", fallbackMethod="fallback")` plus config in `application.yml`. Combine with `@Retryable` and metrics (Micrometer).  
Q: Key annotations?  
